...
* `vpc_type` - (Optional) The type of the VPC. Can be either `regular` or `rdma`. Defaults to `regular`. The `rdma` type creates a RDMA VPC and may not be available to all users.
