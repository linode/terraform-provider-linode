...
func (data *DataSourceModel) ParseLKEType(
	ctx context.Context, lkeType *linodego.LKEType,
) diag.Diagnostics {
...
func (model *LKETypeFilterModel) parseLKETypes(ctx context.Context,
	lkeTypes []linodego.LKEType,
) diag.Diagnostics {
	...
	for i := range lkeTypes {
		...
		diags := m.ParseLKEType(ctx, &lkeTypes[i])
		if diags.HasError() {
			return diags
		}
