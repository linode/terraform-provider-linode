...
func TestParseMaintenancePolicies(t *testing.T) {
	...
	assert.Equal(t, types.StringValue("Migrates the Linode to a new host while it remains fully operational. Recommended for maximizing availability."), data.MaintenancePolicies[0].Description)
	...
	assert.Equal(t, types.StringValue("Powers off the Linode at the start of the maintenance event and reboots it once the maintenance finishes. Recommended for maximizing performance."), data.MaintenancePolicies[1].Description)
	...
	assert.Equal(t, types.StringValue("Custom policy designed to power off and perform maintenance during user-defined windows only."), data.MaintenancePolicies[2].Description)
