					...
					func(s *terraform.State) error {
						...
						for i := 0; i < numAddresses; i++ {
							...
							if rs.Primary.Attributes[prefix+"gateway"] != "" &&
								...
								if !regexp.MustCompile(`\.1$`).
									MatchString(rs.Primary.Attributes[prefix+"gateway"]) {
									return fmt.Errorf(
										"attribute %sgateway has invalid value: %s",
										prefix,
										rs.Primary.Attributes[prefix+"gateway"],
									)
								}

								if !regexp.MustCompile(`.ip.linodeusercontent.com$`).
									MatchString(rs.Primary.Attributes[prefix+"rdns"]) {
									return fmt.Errorf(
										"attribute %srdns has invalid value: %s",
										prefix,
										rs.Primary.Attributes[prefix+"rdns"],
									)
								}
